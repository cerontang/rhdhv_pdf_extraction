...
info = ["SampID", "TCR%", "SCR%", "RQD%"]
info_name = info[0]

VBH_folder_path = r'C:\Users\921722\Box\BI3753 Team\99 - Temp\_SSCT\20230307 - Corehole UCS Samples\logs'
...
for VBH_file in VBH_list:
    ...
    with pdfplumber.open(VBH_file_path) as pdf:
        ...
        for k in range(totalPages):
            ...
            target_pageLocation = textPage.crop(boundingBox, relative=False)
            extractedText = target_pageLocation.extract_table(table_settings={"vertical_strategy": "text", "horizontal_strategy": "text", "intersection_tolerance": 5, "snap_y_tolerance": 7})

            #CUSTOMISATION BLOCK
            # create a nicer raw description list
            if extractedText is None:
                continue
            print(VBH_file, extractedText)
...
#dataframe.to_csv(f'csv/ERI_{info_name}_{date_time}.csv')