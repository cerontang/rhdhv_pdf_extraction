...
VBH_folder_path = r'C:\Users\921722\OneDrive - Royal HaskoningDHV\20220824 AD\AHAM'
    ...
    with pdfplumber.open(VBH_file_path) as pdf:
        ...
        for k in range(totalPages):
            ...
            if sameFormat == 'N':
                ...
                r2 = cv2.selectROI(imResize, fromCenter)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
                # boundingBox = (131.75, 210, 267.46, 719)
                boundingBox2 = (r2[0] * 0.8, r2[1] * 0.8, r2[0] * 0.8 + r2[2] * 0.8, r2[1] * 0.8 + r2[3] * 0.8)
                master_boundingbox2_list.append(boundingBox2)
                #print(boundingBox)
                #########################
            else:
                boundingBox = master_boundingbox_list[0]
                boundingBox2 = master_boundingbox2_list[0]
            target_pageLocation = textPage.crop(boundingBox, relative=False)
            ...
            #######################################################################################################################
            #Extraction of depths and VBH name
            ...
            VBH_pageLocation = textPage.crop(boundingBox2, relative=False)
            VBH_name = VBH_pageLocation.extract_table(table_settings={"vertical_strategy": "text", "horizontal_strategy": "text", "intersection_tolerance": 20})
...
dataframe.to_csv('csv\VBH_GI_Description_AHAM_220824.csv')